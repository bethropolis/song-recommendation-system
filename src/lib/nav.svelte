<script>
  ...
  onMount(() => {
    // Use a $ prefix to access the value of the store.
    const pathName = $active_tab;

    // Check if the pathName is not empty and set 'active' accordingly, otherwise set it to "home".
    active = pathName.slice(1) ? pathName.slice(1) : "home";
  });
</script>

<div class="sidebar bg-transition hidden md:flex bg-white dark:bg-slate-600">
  <a href="#">
    <img class="logo select-none" src="logo.svg" alt="song recommendation" />
  </a>
...
<div class="mobile-navigation bg-white flex md:hidden dark:bg-slate-600">
  <a
    href="/"
    class="icon-link home text-gray-500 dark:text-slate-100"
    class:active={active === "home"}
...
<style>
  .sidebar {
    @apply flex-col items-center w-16 h-screen py-8 space-y-8;
  }
  .mobile-navigation {
    @apply fixed p-2 bottom-0 left-0 right-0  justify-around z-20;
  }
